...
- If the user requests categorization or graph building:
  ...
  - Generate a Cypher `CREATE` or `MERGE` statement with:
    ...
    - Also create Cypher BELONGS_TO relationships between files and their category. If entities are extracted (names, organizations), create nodes and MENTIONS relationships.
- When generating Cypher code to insert documents and categories into Neo4j:
- Always use `MERGE` instead of `CREATE` to avoid duplicates.
- Use `MERGE (f:File {name: ...})` and then `SET f.category = ..., f.pii_flag = ..., f.summary = ...`.
- Do not reuse the same variable name (`f`, `c`, etc.) more than once in the same query.
- For relationships, use `MERGE (f)-[:BELONGS_TO]->(c)`.

Example Cypher output format:
MERGE (f:File {name: "example.pdf"})
SET f.category = "manual", f.pii_flag = false, f.summary = "Example summary"

MERGE (c:Category {name: "manual"})
MERGE (f)-[:BELONGS_TO]->(c)    
- When generating Cypher, group all related MERGE and SET statements for a file and its category together in one block. Do not split Cypher into separate lines or transactions that reference the same variable.
- PII includes: names, addresses, emails, phone numbers, SSNs, medical terms, dates of birth, or government ID numbers.
- If PII is found in a file:
  - Set `f.pii_flag = true`
  - Additionally, `MERGE (p:PiiTag {label: "PII"})` and `MERGE (f)-[:CONTAINS]->(p)` to tag the file node in the graph.
